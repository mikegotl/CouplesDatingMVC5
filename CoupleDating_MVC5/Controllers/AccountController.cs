...
namespace CoupleDating_MVC5.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        // GET: /Account/Login
        [AllowAnonymous]
        public ActionResult Login(string returnUrl, string rtn)
        {
            ViewBag.ReturnUrl = returnUrl;
            Config c = new Config();
            c.LoggedInMember = null;

            //if (String.IsNullOrEmpty(rtn))
            //{
            //    return RedirectToLocal("/stamp");
            //}
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            bool checkIfEmailExists = db.AspNetUsers.Any(m => m.Email == model.Email);

            if (!checkIfEmailExists)
            {
                ModelState.AddModelError("", "Username does not exist in system. Not yet registered.");
                return View(model);
            }
